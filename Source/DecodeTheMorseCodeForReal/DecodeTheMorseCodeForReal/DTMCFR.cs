    ...
    public class DTMCFR
    {
        ...
        public string DecodeChunks2(List<DTMCFRDataChunk> chunks, DTMCFRDataToAnalysis[] arr0, DTMCFRDataToAnalysis[] arr1)
        {
            ...
            chunks.ForEach(x =>
            {
                ...
                {
                    case '0':
                        switch (FindChunkClass(x.Length, arr0))
                        {
                            case 0:
                                break;
                            case 1:
                        ...
                        break;
                    case '1':
                        switch (FindChunkClass(x.Length, arr1))
                        {
                            case 0:
                                res.Append('.');
                                break;
        ...
        public string decodeBitsAdvanced(string bits)
        {
            ...
            clustering.Cluster(arr0, new double[3] { 1.0 / 14.0, 6.0 / 14.0, 13.0 / 14.0 }); //pause between: dots/dashes 1 unit, characters 3 units, words 7 units [7 units long: 1/7 * 1/2, 6/14, 13/14]
            clustering.Cluster(arr1, new double[2] { 1.0 / 6.0, 5.0 / 6.0 });    //dot 1 unit, dash 3 units [3 units long, dot in half of first unit - 1/3 * 1/2, dash in half of last - 5/6]
